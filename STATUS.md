...
Manutenzione: metà settembre (nessuna sessione; scritti tre dialoghi nuovi e sciolta la
contraddizione sui benchmark tra questo file e il README dello specchio pubblico)
Start: 2026-08-02
...
Vale in TUTTE E TRE le modalità: sessione nel repo, pacchetto portatile, e chat voce che
legge lo specchio pubblico. Anzi, la modalità voce è quella preferita, perché il benchmark
misura il parlato. L'unica condizione in modalità voce o portatile è che il blocco punteggi
di programma.md torni dentro il post-mortem, altrimenti la misura non esiste. Se la sessione
di oggi è a voce, corri il benchmark lì e non aspettare una sessione nel repo.

Dopo il benchmark, se resta tempo, il dialogo del giorno. Non invertire l'ordine: il
benchmark deve essere la prima cosa che dice in italiano oggi, senza riscaldamento
prolungato, altrimenti la baseline è gonfiata.
...
## Note
- Prossima volta: 1) BENCHMARK (vedi sopra), 2) ripasso a freddo di cibo-01 senza anteprima, 3) decidere la ricalibrazione delle fasi: 11 giorni utili, proposta = sopravvivenza pura (cibo, trasporti, frasi di riparazione) + simulazione finale corta, saltando fase 2 e 4 come etichette.
- Dialoghi pronti, in quest'ordine consigliato dopo il benchmark: `trasporti-01-check-in-appartamento` (ascolto e riparazione, il Lei per tutta la scena), `negozi-01-mercato-a-peso` (il drill vero del `ne` e delle quantità, più `mi dia` contro "dammi"), `cibo-02-gelateria` (veloce, complicazione dentro la scena, `piacere` e i verbi in `-isc-`). Nessuno dei tre tocca gli scenari congelati dei benchmark, di proposito: ristorante affollato in cinque, banco autonoleggio a Olbia e small talk al bar restano senza impalcatura, altrimenti i benchmark 3 e 4 non misurano più niente.
- Ancora senza dialogo, se servono altre sessioni: farmacia, biglietti del traghetto, problema in camera (acqua calda), presentarsi e presentare il gruppo.
